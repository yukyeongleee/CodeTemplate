...
class FaceShifterLoss(LossInterface):
    def get_loss_G(self, G_dict):
        ...
        # Id loss
        if self.args.W_id:
            L_id = Loss.get_id_loss(G_dict["I_source_id"], G_dict["I_swapped_id"])
            L_G += self.args.W_id * L_id
            self.loss_dict["L_id"] = round(L_id.item(), 4)

        # Attribute loss
        if self.args.W_attr:
            L_attr = Loss.get_attr_loss(G_dict["I_target_attr"], G_dict["I_swapped_attr"], self.args.batch_per_gpu)
            L_G += self.args.W_attr * L_attr
            self.loss_dict["L_attr"] = round(L_attr.item(), 4)

        # Reconstruction loss
        if self.args.W_recon:
            L_recon = Loss.get_L2_loss_with_same_person(G_dict["I_target"], G_dict["I_cycle"], G_dict["same_person"], self.args.batch_per_gpu)
            L_G += self.args.W_recon * L_recon
            self.loss_dict["L_recon"] = round(L_recon.item(), 4)
        
        # Cycle loss
        if self.args.W_cycle:
            L_cycle = Loss.get_L2_loss(G_dict["I_target"], G_dict["I_cycle"])
